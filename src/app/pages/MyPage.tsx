...
export function MyPage() {
  ...
  const { user, displayName, hostApplication, isHost, isAdmin } = useAuth();
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="max-w-[1280px] lg:max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 py-5 lg:py-8">
        ...
        {/* 관리자 전용 — 일반 기능 카드와 섞이지 않도록 별도 톤(진회색)의
            독립 배너로 둔다. 여기서 클릭하면 관리자 콘솔(/admin)로 완전히
            넘어가고, 관리자 관련 다른 메뉴는 마이페이지에 두지 않는다. */}
        {isAdmin && (
          <button
            onClick={() => go("/admin")}
            className="w-full flex items-center justify-between gap-3 rounded-2xl px-5 py-4 lg:px-6 lg:py-5 mb-4 lg:mb-6 bg-gray-900 hover:bg-gray-800 transition-colors text-left"
          >
            <div className="flex items-center gap-3 lg:gap-4 min-w-0">
              <div className="w-9 h-9 lg:w-12 lg:h-12 rounded-xl bg-white/10 flex items-center justify-center shrink-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs lg:text-sm mt-0.5 text-gray-400 truncate">관리자 콘솔에서 운영 현황을 확인하고 콘텐츠를 관리하세요</p>
              </div>
            </div>
            <ChevronRight className="w-4 h-4 lg:w-5 lg:h-5 shrink-0 text-gray-400" strokeWidth={2} />
          </button>
        )}
